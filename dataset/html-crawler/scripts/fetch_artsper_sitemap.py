#!/usr/bin/env python3
"""Fetch Artsper URLs from sitemap.xml and select new/updated entities only."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fetch Artsper artist/artwork URLs from sitemap.xml")
    parser.add_argument("--index", default=DEFAULT_INDEX, help="Sitemap index URL")
    ...
    parser.add_argument("--known-db-url", default=None, help="Also treat catalog_* external_id as known")
    parser.add_argument(
        "--state",
        type=Path,
        default=Path("state/sitemap_lastmod.json"),
        help="Persist lastmod per entity for update detection",
    )
    parser.add_argument(
        "--out-all",
        type=Path,
        default=Path("state/sitemap_urls.txt"),
    ...
    parser.add_argument(
        "--report",
        type=Path,
        default=Path("state/sitemap_diff.json"),
...
def main() -> None:
    ...
    entries = fetch_artsper_sitemap_entries(args.index, concurrency=args.concurrency)
    ...
    result.stats.child_sitemaps = 0  # filled below only for report context
