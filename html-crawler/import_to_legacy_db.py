...
INSERT INTO public.arts_artists (
  id, artist_name, profile_image_url, biography, artist_slug, artist_url, path
) VALUES (
  %(id)s, %(artist_name)s, %(profile_image_url)s, %(biography)s, %(artist_slug)s,
  %(artist_url)s, %(path)s
)
...
INSERT INTO public.arts_artworks (
  ...
  price, currency, artwork_cover_url, artist_slug, artwork_url, path
) VALUES (
  %(id)s, %(artwork_title)s, %(artwork_year)s, %(artist_name)s, %(category)s, %(medium)s,
  %(price)s, %(currency)s, %(artwork_cover_url)s, %(artist_slug)s, %(artwork_url)s, %(path)s
)
ON CONFLICT (id) DO UPDATE SET
    ...
    *,
    ...
    try:
        with connection.cursor() as cursor:
            if skip_existing and _legacy_exists(cursor, entity_type, entity_id):
                stats.skipped += 1
                return
            tracked_fields = TRACKED_FIELDS[("artsper", entity_type)]
            if entity_type == "artist":
                select_sql = (
                    f"SELECT {', '.join(tracked_fields)} FROM public.arts_artists WHERE id = %s"
                )
                upsert_sql = UPSERT_ARTIST_SQL
            else:
                select_sql = (
                    f"SELECT {', '.join(tracked_fields)} FROM public.arts_artworks WHERE id = %s"
                )
            ...
            inserted = upsert_with_history(
                ...
                metadata={
                    ...
                    "artwork_cover_url": row.get("artwork_cover_url"),
                },
            )
        connection.commit()
        if inserted:
