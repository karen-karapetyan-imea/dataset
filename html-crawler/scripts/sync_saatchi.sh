...
HISTORY_DDL_FILE="${HISTORY_DDL_FILE:-$ROOT/sql/006_entity_history.sql}"
...
if [[ "${EXTRACT_ONLY:-0}" != "1" && "${IMPORT_ONLY:-0}" != "1" ]]; then
  ...
  if [[ ! -f "$HISTORY_DDL_FILE" ]]; then
    echo "History DDL file not found: $HISTORY_DDL_FILE" >&2
    exit 1
  fi
  echo "[sync_saatchi] apply ddl=$DDL_FILE"
  psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -f "$DDL_FILE"
  echo "[sync_saatchi] apply history ddl=$HISTORY_DDL_FILE"
  psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -f "$HISTORY_DDL_FILE"
fi
